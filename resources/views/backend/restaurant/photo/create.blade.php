...
@section('body')
    <div class="ibox">
        <div class="ibox-head">
            <div class="ibox-title mx-auto text-black-50">Create Photo</div>
            ...
            <form class="form-horizontal" action="{{route('photo.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="condition" class="form-control @error('condition') is-invalid @enderror">
                            <option value="interior"{{old('condition'== 'interior' ? 'selected' : '')}}>Interior</option>
                            <option value="exterior" {{old('condition'== 'exterior' ? 'selected' : '')}}>Exterior</option>
                        </select>
                        @error('condition')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="form-group row">
                    <div class="col-sm-10 ml-sm-auto">
                        <button class="btn btn-info " type="submit">Create new photo</button>
                    </div>
                </div>
            </form>
        </div>
